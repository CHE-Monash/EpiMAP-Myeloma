...
program define impute_bcr
	...
	// L5 response collapsed to CR vs not-CR, for L5_TFI. The six-level block was carrying one binary
	// contrast on n = 302 and emptied a level across imputations. min() so the engine's clamp matches.
	cap drop bcr_grp_l5
	qui mi passive: gen byte bcr_grp_l5 = min(BCR_L5, 2) if !mi(BCR_L5)
...
program define finalise_mi
	...
	keep ID ID_BS Event* Date* Age* Male ECOGcc ISS RISS SCT MNT MND_L1 MNR_L1 refr_line refr_len_tx_in refr_len_mnt_in refr_len_mnt_l1 bcr_grp_l1 bcr_grp_l5 L7plus CM* BCR* pBCR* Reg* OS Line Duration CID CLine CStart CEnd Country F_* CN_* Year Albumin SerumB2Microglobulin LactateDehydrogenase LDHUpperLimit LDHRisk FISHRisk eGFR _* Bortezomib Carfilzomib Cisplatin Cyclophosphamide Daratumamab Dexamethasone Doxorubicin Elotuzamab Etoposide Lenalidomide Melphalan Methylprednisolone Panobinostat Prednisolone Thalidomide Pomalidomide Ixazomib TXD* TFI*
	sort ID_BS Date0
