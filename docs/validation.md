...
The latest deterministic point-estimate run (v3.0, July 2026) scores **86.5% (147/170)** out of sample; the *same* model checked in-sample scores **93.1% (162/174)**. That is the best out-of-sample score this model has recorded, and it followed the maintenance-duration rework — drawing maintenance before the gap and truncating the gap to contain it (`refractory.md` §5(7)). The gap between the two folds is the point: a check that fails out of sample and passes in-sample is a 70/30 sampling artefact, not a defect, and most of the residual is that. The L2–L4 treatment-duration misses are the clearest case, failing out of sample and passing in-sample on the same equations.
...
**One is worth reading as real signal.** Maintenance duration still runs short for lenalidomide in-sample — 17.9 months against a registry Kaplan–Meier median of 24.6 — though both regimens now pass out of sample and thalidomide passes in both folds. Three explanations were tested and excluded: the fitted equation is correct, the simulated and registry case-mixes are near-identical, and death curtailment accounts for under half a month. The residual is recorded in `refractory.md` §5(7) rather than resolved.
